...
Item {
    ...
    function refresh() {
        ...
        backendError = "";
        listProc.stdout.buf = "";
        listProc.command = ["bash", "-lc", "command -v cliphist >/dev/null 2>&1 || exit 127; " + "out=$(cliphist list 2>&1); ec=$?; " + "if [ $ec -ne 0 ]; then " + "if printf '%s' \"$out\" | grep -qi 'please store something first'; then exit 0; fi; " + "printf '%s\\n' \"$out\"; exit $ec; fi; " + "printf '%s\\n' \"$out\""];
    ...
    function applyFilter() {
        ...
        if (q === "")
            ...
            filtered = entries.filter(function(e) {
                return (e.preview || "").toLowerCase().indexOf(q) !== -1 || (e.id || "").toLowerCase().indexOf(q) !== -1;
            });
    ...
    function updatePreviewForSelected() {
        ...
        const line = shellQuote(entry.raw);
        const cmd = "line=" + line + "; " + "tmp=$(mktemp /tmp/qs-clip-prev.XXXXXX.bin) || exit 1; " + "if printf '%s\\n' \"$line\" | cliphist decode > \"$tmp\" 2>/dev/null; then " + "mime=$(file --mime-type -b \"$tmp\" 2>/dev/null || true); " + "if [[ \"$mime\" == image/* ]]; then " + "printf '__IMG__\\n%s\\n%s\\n' \"$mime\" \"$tmp\"; " + "else rm -f \"$tmp\"; printf '__NOIMG__'; fi; " + "else rm -f \"$tmp\"; printf '__NOIMG__'; fi";
    ...
    Process {
        ...
        command: ["bash", "-lc", "out=$(cliphist list 2>&1); ec=$?; " + "if [ $ec -ne 0 ]; then " + "if printf '%s' \"$out\" | grep -qi 'please store something first'; then exit 0; fi; " + "printf '%s\\n' \"$out\"; exit $ec; fi; " + "printf '%s\\n' \"$out\""]
        running: false
        onExited: (exitCode) => {
            ...
            if (exitCode !== 0) {
                ...
                service.backendError = err !== "" ? ("Clipboard backend error:\n" + err) : "cliphist is not available. Install and start cliphist daemon.";
    ...
    Process {
        ...
        command: ["bash", "-lc", "missing=''; " + "for c in cliphist wl-copy file base64; do " + "command -v \"$c\" >/dev/null 2>&1 || missing=\"$missing $c\"; " + "done; " + "missing=${missing# }; " + "if [ -z \"$missing\" ]; then echo __OK__; exit 0; fi; " + "echo \"$missing\"; exit 1"]
        running: false
        onExited: (exitCode) => {
