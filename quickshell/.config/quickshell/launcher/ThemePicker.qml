...
USER_DIR="$HOME/.config/themes/themeLists"
CURRENT=$(cat "$HOME/.config/themes/current/theme.name" 2>/dev/null)
SEEN=""
...
'
    ...
    function applyTheme(name) {
        var proc = Qt.createQmlObject('import Quickshell.Io; Process { command: ["bash","-c",""]; running: false }', root, "applyProc" + (++_cmdSeq));
        proc.onExited.connect(function() {
            proc.destroy();
        });
        proc.command = ["bash", "-c", "theme-set '" + name + "'"];
        proc.running = true;
        root.showing = false;
    }
                            ...
                            Rectangle {
                                ...
                                Row {
                                    ...
                                    Text {
                                        anchors.verticalCenter: parent.verticalCenter
                                        text: tdata.name || ""
                                        color: isSelected ? (theme.accent || "#89b4fa") : (tdata.fg || "#cdd6f4")
                                        font.pixelSize: 9
                                        font.family: "JetBrainsMono Nerd Font"
