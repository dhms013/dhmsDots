...
Item {
    ...
    // Injected by dhms-shell; defaults to the session DHMSDOTS_PATH.
    property string dotsPath: Quickshell.env("DHMSDOTS_PATH")
    property string stateHome: Quickshell.env("HOME") + "/.local/state"
    // Newline-separated list of directories the picker scans. Mirrors the old
    // quickshell: theme-bundled backgrounds plus the personal collection.
    property string imageDirs: Quickshell.env("DHMS_IMAGE_SELECTOR_DIRS") || Quickshell.env("DHMS_IMAGE_SELECTOR_DIR") || Quickshell.env("DHMS_STOCK_BACKGROUNDS_DIR") || (home + "/.config/themes/current/theme/backgrounds\n" + home + "/Pictures/backgrounds")
    property string imageRows: ""
    property string loadedImageRows: ""
    property string selectionFile: Quickshell.env("DHMS_IMAGE_SELECTOR_SELECTION_FILE") || Quickshell.env("DHMS_BACKGROUND_SELECTION_FILE")
    property string selectedImage: Quickshell.env("DHMS_IMAGE_SELECTOR_SELECTED")
    ...
    // Live-preview mode for the surrounding desktop: "background" re-points the
    // wallpaper (in-memory, via `dhms-shell background setInstant`), "theme" also
    // repaints shell colors/surfaces (via `dhms-shell shell applyTheme`), "none"
    // disables the hooks. Nothing here commits — the committed theme and the
    // current-background symlink are untouched until apply/select.
    property string previewMode: "background"
    // Last path (background mode) or theme name (theme mode) already previewed,
    // so repeated selections don't re-apply; cleared when the picker closes.
    property string lastPreviewed: ""
    // Let the live wallpaper show through the scrim while browsing; keep the
    // scrim opaque for preview-less pickers.
    readonly property real scrimOpacity: previewMode === "none" ? 1 : 0.6
    // Preview IPC runs through one serial queue so the restore, enqueued on
    ...
    property string pendingOpenSelFile: ""
    property string pendingOpenDoneF: ""
    property bool pendingOpenLabels: false
    ...
    function revealWhenSettled(serial) {
        Qt.callLater(function() {
            if (serial === root.requestSerial && root.opened && root.imagesLoaded && root.imageArray.length > 0) {
                ...
                root.focusPicker();
                // Kick the first live preview (committed wallpaper / current theme)
                // once the carousel settles, since selection may not have changed.
    ...
    function currentPath() {
        if (imageArray.length === 0 || !itemMatches(selectedIndex))
            return "";
    ...
    // Theme picker rows point at cache previews like
    // ~/.cache/dhms/theme-selector/previews/<theme>.<ext>, so the basename minus
    // extension is the theme name the render script receives.
    ...
    function previewSelected() {
        ...
        if (previewMode === "theme") {
            ...
            if (!theme || theme === lastPreviewed)
                return ;

            lastPreviewed = theme;
            pendingRenderTheme = theme;
            themeRenderProc.command = ["dhms-theme-render", theme];
            themeRenderProc.running = true;
        } else if (previewMode === "background") {
            if (path === lastPreviewed)
                return ;

            lastPreviewed = path;
            queuePreview(["dhms-shell", "-q", "background", "setInstant", path]);
        }
    }

    function applyRenderedTheme(output) {
        // Ignore renders that finish after the picker closed or after the user
        // moved on to another theme; restorePreview restores the committed state.
        if (!opened || pendingRenderTheme !== currentThemeName())
            return ;
        ...
        var colors = (parts[0] || "").trim();
        var shell = (parts[1] || "").trim();
        if (colors || shell)
            queuePreview(["dhms-shell", "-q", "shell", "applyTheme", colors, shell]);
    ...
    function queuePreview(command) {
        previewQueue.push(command);
        runNextPreview();
    }

    function runNextPreview() {
        if (previewQueue.length === 0 || previewProc.running)
            return ;

        previewProc.command = previewQueue.shift();
        previewProc.running = true;
    }

    function restorePreview() {
        if (previewMode === "theme") {
            // Re-apply the committed theme's palette in-memory and reset the
            // wallpaper from the current-background link. Both stay in-memory, so a
    ...
    function select(index, immediate) {
        ...
        if (index === selectedIndex && immediate !== true)
            return ;
    ...
    function releaseNextDoneFile() {
        if (releaseProc.running || doneFilesToRelease.length === 0)
            return ;
    ...
    function applySelected() {
        ...
        // Direct summon (keybind/menu) has no caller-provided selection file.
        // Mirror omarchy's flow: persist via the current-background symlink and
        // push the live swap over IPC immediately (see applyExternal).
        if (!selectionFile) {
            applyExternal(currentPath());
            cancel();
            return ;
        }
    ...
    function openSelector(nextImageDirs, nextImageRows, nextSelectedImage, nextSelectionFile, nextDoneFile, nextShowLabels, nextFilterable, nextPreviewMode) {
        ...
        previewMode = String(nextPreviewMode || "background");
        lastPreviewed = "";
        pendingRenderTheme = "";
        filterText = "";
        layoutSettled = false;
    ...
    // Lifecycle hooks invoked by dhms-shell summon/hide. shell.summon(id,
    ...
    function open(payload) {
        ...
        // Only trust an explicit payload selection. Falling back to the mutable
        // selectedImage property here leaks the previous session's focus into a
        // bare summon ('{}'), which would sidestep the committed-background resolve.
        var sel = args.selectedImage !== undefined ? String(args.selectedImage || "") : "";
        ...
        var mode = String(args.previewMode || "");
        if ((mode === "" || mode === "background") && !sel) {
            // Background summon without an explicit selection: focus the committed
            // wallpaper so the carousel opens on what the desktop shows. Resolve the
            // current-background symlink to the real path the scan lists, then open.
            root.pendingOpenDirs = dirs;
            root.pendingOpenRows = rows;
            root.pendingOpenSelFile = selFile;
            root.pendingOpenDoneF = doneF;
            root.pendingOpenLabels = labels;
            root.pendingOpenFilter = filter;
            root.pendingOpenMode = mode;
            // list.sh emits symlink-prefixed paths (find -L keeps the dir prefix),
            // so match with single readlink — readlink -f would physicalize through
            // ~/.config/themes and never equal a scanned row.
            var commitLink = Util.shellQuote(home + "/.config/themes/current/background");
            resolveCommitProc.command = ["bash", "-c", "p=$(readlink " + commitLink + " 2>/dev/null); " + "[[ -n $p ]] || p=$(readlink -f " + commitLink + " 2>/dev/null); " + "printf '%s\\n' \"$p\""];
            resolveCommitProc.running = true;
            return ;
        }
        openSelector(dirs, rows, sel, selFile, doneF, labels, filter, mode);
    }
    ...
    function applyExternal(path) {
        path = String(path || "").trim();
        if (!path)
            return ;

        console.info("ImagePicker applying selection:", path);
        var quoted = Util.shellQuote(path);
        var link = Util.shellQuote(home + "/.config/themes/current/background");
        externalApplyProc.command = ["bash", "-c", "ln -nsf " + quoted + " " + link + " && dhms-shell background setInstant " + quoted];
        externalApplyProc.running = true;
    }
    ...
    Timer {
        ...
        interval: 90
        repeat: false
        onTriggered: root.previewSelected()
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            waitForEnd: true
            onStreamFinished: root.applyRenderedTheme(String(text || "").trim())
        }

        stderr: StdioCollector {
            onStreamFinished: {
                if (String(text || "").trim()) {
                    console.warn("ImagePicker theme render:", String(text).trim());
                }
            }
        }
    ...
    Process {
        id: resolveCommitProc

        stdout: StdioCollector {
            waitForEnd: true
            onStreamFinished: root.openSelector(root.pendingOpenDirs, root.pendingOpenRows, String(text || "").trim(), root.pendingOpenSelFile, root.pendingOpenDoneF, root.pendingOpenLabels, root.pendingOpenFilter, root.pendingOpenMode)
        }

        stderr: StdioCollector {
            onStreamFinished: {
                if (String(text || "").trim()) {
                    console.warn("ImagePicker resolve:", String(text).trim());
                }
            }
        }
    ...
    Process {
        ...
        onExited: root.runNextPreview()

        stderr: StdioCollector {
            onStreamFinished: {
                if (String(text || "").trim()) {
                    console.warn("ImagePicker preview:", String(text).trim());
                }
            }
        }
    ...
    // Applies a pick when no caller-provided selection file exists (direct
    // summons). Mirrors omarchy-theme-bg-set: repoint the current-background
    // symlink, then push the live swap over IPC. A real QML Process here —
    // Quickshell.execDetached silently does nothing on quickshell 0.3.1.
    Process {
        id: externalApplyProc

        stdout: StdioCollector {
            onStreamFinished: {
                if (String(text || "").trim()) {
                    console.warn("ImagePicker apply:", String(text).trim());
                }
            }
        }
        ...
        stderr: StdioCollector {
            onStreamFinished: {
                if (String(text || "").trim()) {
                    console.warn("ImagePicker apply stderr:", String(text).trim());
                }
            }
        }
    ...
    PanelWindow {
        ...
        visible: root.opened
        color: "transparent"
        WlrLayershell.namespace: "dhms-image-selector"
        ...
        Rectangle {
            ...
            color: Util.alpha(root.scrim, root.scrimOpacity)
        }
