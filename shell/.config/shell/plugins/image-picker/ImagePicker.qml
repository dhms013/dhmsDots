...
Item {
  ...
  Process {
    ...
    stdout: StdioCollector {
      onStreamFinished: if (String(text || "").trim()) console.warn("ImagePicker apply:", String(text).trim())
    }
    stderr: StdioCollector {
      onStreamFinished: if (String(text || "").trim()) console.warn("ImagePicker apply stderr:", String(text).trim())
    }
  }
  ...
  function applyExternal(path) {
    ...
    externalApplyProc.command = ["bash", "-c",
      "ln -nsf " + quoted + " " + link +
      " && dhms-shell background setInstant " + quoted]
