...
function normalizeItem(id, raw) {
  ...
  return {
    ...
    // hidden:true keeps a node routable ({"menu":"id"} payloads) while never
    // rendering it in its parent's listing or search — e.g. an Apps view
    // bound to SUPER+SPACE that must not appear inside the custom menu.
...
// against `dhms-default-browser` -- so the batch runs it once and the rows
...
var GUARD_READERS = [
  "dhms-channel-current",
  "dhms-default-agent",
  "dhms-default-browser",
  "dhms-default-editor",
  "dhms-default-terminal",
  "setup-dns current"
]
...
function guardHelpers() {
  return 'declare -A __dhms_pkgs=()\n'
    + 'mapfile -t __dhms_pkg_names < <({ pacman -Qq; LC_ALL=C pacman -Qi'
    + " | awk '/^[A-Za-z]/ { provides = ($0 ~ /^Provides/); sub(/^[^:]*: /, \"\") }"
    + ' provides && $0 != "None" { n = split($0, p, " ");'
    + ' for (i = 1; i <= n; i++) { sub(/[<>=].*/, "", p[i]); print p[i] } }\'; } 2>/dev/null)\n'
    + 'for __dhms_pkg in "${__dhms_pkg_names[@]}"; do __dhms_pkgs[$__dhms_pkg]=1; done\n'
    + '__dhms_pkg_has() { [[ -n ${__dhms_pkgs[$1]-} ]] && return 0; '
    + '[[ $1 == *[\\<\\>=]* ]] && { pacman -Q "$1" &>/dev/null; return; }; return 1; }\n'
    + 'dhms-pkg-present() { local p; for p in "$@"; do __dhms_pkg_has "$p" || return 1; done; return 0; }\n'
    + 'dhms-pkg-missing() { local p; for p in "$@"; do __dhms_pkg_has "$p" || return 0; done; return 1; }\n'
    + 'dhms-cmd-present() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 1; done; return 0; }\n'
    + 'dhms-cmd-missing() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 0; done; return 1; }\n'
}
...
function guardPrelude(guards) {
  ...
  for (var i = 0; i < GUARD_READERS.length; i++) {
    ...
    prelude += "__dhms_read_" + i + "=$(" + GUARD_READERS[i] + " 2>/dev/null) || :\n"
  }
