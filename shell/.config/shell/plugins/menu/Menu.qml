...
Item {
    ...
    // Injected by dhms-shell when this plugin is summoned.
    property string dotsPath: Quickshell.env("DHMSDOTS_PATH")
    property var shell: null
    property var manifest: null
    // Plugin lifecycle hooks. The host calls open(payloadJson) after
    // `dhms-shell shell summon dhms.menu ...` and close() when hidden.
    property string pendingInitialMenu: "root"
    property string fontFamily: Style.font.menuFamily
    // Single JSONC menu definition, kept inside the plugin directory so it
    // version-controls with the rest of the shell. There is deliberately no
    // user-extension merge layer in this setup.
    property string defaultMenuPath: Quickshell.shellDir + "/plugins/menu/menu.jsonc"
    ...
    // Optional shell.json override: "menu": { "maxRows": <n> }. Caps the list
    // viewport to n rows — extra entries scroll inside the card instead of
    // growing it. 0/unset keeps the old hug-the-content sizing.
    readonly property var menuConfig: shell && shell.shellConfig && shell.shellConfig.menu ? shell.shellConfig.menu : ({})
    readonly property int maxVisibleRows: Number(menuConfig.maxRows) > 0 ? Math.floor(Number(menuConfig.maxRows)) : 0
    ...
    // re-runs every time its submenu is entered, so a theme added since the
    // shell started shows up without restarting it.
    readonly property var providers: ({
        "themes": {
            "script": "current=$(cat \"$HOME/.config/themes/current/theme.name\" 2>/dev/null); for f in \"$HOME\"/.config/themes/themeLists/*; do [[ -d $f ]] || continue; t=$(basename \"$f\"); printf '%s\\t%s\\t%s\\n' \"$t\" \"$t\" \"$current\"; done",
            "icon": "\ue793",
            "volatile": true,
            "actionFor": function(value) {
                return "theme-set " + Util.shellQuote(value);
            }
        },
        "power-profiles": {
    ...
    }) // id → true|false (allow visibility)
    property var checkedResults: ({
    }) // id → true|false (show ✓)
    property var disabledResults: ({
    }) // id → true|false (dim, skip cursor)
    ...
    function availableRowsHeight() {
        ...
        // Skipped when menu.maxRows is configured — a fixed-viewport menu is
        // supposed to grow to its cap on every level, not inherit the opening
        // view's (smaller) size.
        if (panel.maxRowsHeight >= 0 && root.maxVisibleRows <= 0)
    ...
    function rowListHeight(_serial, _count, _filter, _divider) {
        ...
        // With maxRows set, only the first n rows feed the height: the card
        // becomes a fixed 5-row (say) viewport and the rest scroll inside it.
        var rows = displayModel.count;
        if (root.maxVisibleRows > 0 && rows > root.maxVisibleRows)
            rows = root.maxVisibleRows;
    ...
    // Single source: the in-tree menu.jsonc. Kept as a function so future
    // layers can hook in without touching callers.
    ...
    function rebuildDisplay() {
        ...
        if (query) {
            ...
            var searchSort = function searchSort(a, b) {
                if (a.score !== b.score)
    ...
    // `dhms-shell shell summon dhms.menu '{"menu":"system"}'`.
    ...
    function openRoute(initialMenu) {
        ...
        // a leaf, e.g. `dhms menu summon screenrecord-stop`), run it directly
    ...
    // The JSONC source is watched so live edits to the in-tree menu file take
    ...
    PanelWindow {
        ...
        color: "transparent"
        WlrLayershell.namespace: "dhms-menu"
