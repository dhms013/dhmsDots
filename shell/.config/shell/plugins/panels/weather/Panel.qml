...
Panel {
  id: root
  moduleName: "dhms.weather"
  ipcTarget: "dhms.weather"
  manageIpc: false
  ...
  // dhms-weather-location). The query is the wttr.in path segment
  ...
  property FileView locationFile: FileView {
    path: Quickshell.env("HOME") + "/.local/state/dhms/settings/weather.json"
    watchChanges: true
    printErrors: false
  ...
  function persistLocation(name, latitude, longitude) {
    if (name && latitude !== null && longitude !== null)
      locationSaveProc.command = ["dhms-weather-location", "--set", name, latitude + "," + longitude]
    else if (name)
      locationSaveProc.command = ["dhms-weather-location", "--set", name]
    else
      locationSaveProc.command = ["dhms-weather-location", "--clear"]
  ...
  // Mirrors dhms-weather-icon's wttr.in code → nerd-font glyph mapping.
