...
// dhms-network-qr, which emits the interface, security, and SSID it
...
Item {
  ...
  property string dotsPath: Quickshell.env("DHMSDOTS_PATH") || Quickshell.env("HOME") + "/.dhmsDots"
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "dhms.wifiqr")
    else close()
  }
  ...
  function generate(requestedIface) {
    ...
    qrProc.command = requestedIface
      ? ["dhms-network-qr", "--meta", requestedIface]
      : ["dhms-network-qr", "--meta"]
    qrProc.running = true
  }
  ...
  function togglePassword() {
    ...
    pwProc.command = ["dhms-network-password", iface]
    pwProc.running = true
  }
  ...
  PanelWindow {
    ...
    WlrLayershell.namespace: "dhms-network-qr"
