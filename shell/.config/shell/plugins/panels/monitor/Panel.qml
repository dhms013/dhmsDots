...
Panel {
  id: root
  moduleName: "dhms.monitor"
  ipcTarget: "dhms.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (dhms-display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["dhms-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function showBrightnessOsd(percent) {
    ...
    bar.shell.summon("dhms.osd", JSON.stringify({
      icon: "brightness",
      value: percent
    }))
  }
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "dhms-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["dhms-display-text-size", String(px)]
  ...
  Process {
    ...
    // `dhms-brightness-display` races the hardware/driver and can
