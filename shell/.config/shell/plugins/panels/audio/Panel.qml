...
Panel {
  id: root
  moduleName: "dhms.audio"
  ipcTarget: "dhms.audio"

  readonly property var sink: Pipewire.defaultAudioSink
  readonly property var source: Pipewire.defaultAudioSource
  readonly property var nodes: Pipewire.nodes ? Pipewire.nodes.values : []
  readonly property var mprisPlayers: Mpris.players ? Mpris.players.values : []
  readonly property var mediaService: bar?.shell?.firstPartyServiceFor("dhms.media")
  ...
  readonly property var candidateStreams: {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (String(n.name || "").indexOf("dhms_speaker_tuning") === 0) continue
      list.push(n)
    }
    return list
  }
  ...
  //
  // dhms-audio-output-sink resolves the *current* default output through any
  ...
  Process {
    ...
    command: ["dhms-audio-sink-availability"]
  ...
  Process {
    id: volumeSinkProc
    command: ["dhms-audio-output-sink"]
    stdout: StdioCollector {
      waitForEnd: true
