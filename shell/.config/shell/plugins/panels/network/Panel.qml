...
Panel {
  ...
  function summonSpeedTest() {
    ...
    bar.shell.summon("dhms.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }

  function dnsCommand(provider) {
    var command = "setup-dns"
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "floating-terminal"
      root.bar.run(launcher + " " + Util.shellQuote("setup-dns Custom"))
      root.close()
      return
    }
