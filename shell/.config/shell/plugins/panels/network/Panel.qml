...
Panel {
    // { iface, type, ip, prefix, gateway, speed, duplex, ssid, signal, freq, bitrate, rx_bytes, tx_bytes, router_ping_ms, internet_ping_ms }
    ...
    // `dhms-network-status --verbose` samples (~1.5s apart via detailsPoll).
    ...
    // Wi-Fi band state from `dhms-network-band`. `bandCurrent` is the band
    ...
    // The share card is its own panel plugin (dhms.wifiqr) so a replacement
    ...
    function summonWifiQr(forceDetect) {
        ...
        bar.shell.summon("dhms.wifiqr", JSON.stringify(payload));
    }
    ...
    function refresh(scanWifi) {
        ...
        if (!dnsProc.running) {
            // `setup-dns` picks a provider interactively; `setup-dns current` is the
            // read-only probe that prints the active provider for the pills.
            dnsProc.command = ["bash", "-c", "setup-dns current"];
            dnsProc.running = true;
        }
        if (!bandProc.running) {
            bandProc.command = ["dhms-network-band"];
    ...
    function setBand(band) {
        ...
        actionProc.command = ["dhms-network-band", band];
        actionProc.running = true;
    }

    // The speed test is its own panel plugin (dhms.speedtest) so a
    ...
    function summonSpeedTest() {
        ...
        if (info.type === "wifi")
            ...
            connection = "Ethernet";
        bar.shell.summon("dhms.speedtest", connection ? JSON.stringify({
...
// Setup-dns needs root and a TTY for sudo, so every pill hands off to a
  ...
  function setDns(provider) {
    ...
    moduleName: "dhms.network"
    ipcTarget: "dhms.network"
    ...
    // Action runner for band changes. Wi-Fi actions use the
