...
Panel {
  id: root
  moduleName: "dhms.bluetooth"
  ipcTarget: "dhms.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/dhms-bluetooth-device;
  ...
  function deviceCommand(action, address) {
    return ["dhms-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via dhms-bluetooth-device.
  ...
  // the adapter came back on at the next boot. dhms-bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["dhms-bluetooth-power", adapter.enabled ? "off" : "on"])
  }

  IpcHandler {
    target: "dhms.bluetooth"
