...
Item {
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "dhms.emojis")
  }
  ...
  function applySelected(emoji) {
    if (!emoji) return
    root.dismiss()
    // Copy to clipboard; no wtype/ydotool in this setup, so typing into the
    // focused window is not available — the emoji lands on the clipboard.
    Quickshell.execDetached(["bash", "-c",
      "printf %s " + Util.shellQuote(String(emoji)) + " | wl-copy && notify-send -t 1500 \"Emoji copied\" " + Util.shellQuote(String(emoji))])
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "dhms-emojis"
