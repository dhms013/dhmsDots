// Notification service for the dhms shell.
...
Item {
  ...
  // Injected by dhms-shell (the first-party service loader).
  ...
  readonly property string stateDir: home + "/.local/state/dhms/"
  ...
  PersistentProperties {
    id: persisted
    reloadableId: "dhms-notifications"
  ...
  function durationFor(urgency, expireTimeout, app, appIcon) {
    // Chromium escalates web requireInteraction notifications to
    // urgency=critical on D-Bus, which would render as a never-expiring
    // toast. Only Brave does this for routine chat pings — demote those to
    // the normal lifetime; every other sender's critical stays sticky.
    if (urgency === NotificationUrgency.Critical && NotificationLogic.isBraveSender(app, appIcon))
  ...
  // and rare.
  //   - dhms-action: a user-action confirmation toast ("Theme changed",
  ...
  //     Trusted because it's almost always dhms or system shell scripts —
  ...
  //   - app_name is "dhms-action" (dhms's own user-action toasts —
  ...
  // Run the popup's click action, then dismiss. dhms's own toasts carry the
  ...
  // sender. Case-insensitive class matching runs inline over `hyprctl -j
  // clients` (replaces omarchy's helper script).
  function focusApp(entry) {
    if (!entry || !entry.app) return
    var script = "addr=$(hyprctl -j clients 2>/dev/null | jq -r --arg c "
      + Util.shellQuote(String(entry.app))
    ...
    focusAppProc.command = ["bash", "-c", script]
  ...
  // toasts survive shell restarts (notably the restart `dhms-update`
  ...
  function flushSettings() {
    ...
    // setText only stages on quickshell 0.3.x; without writeAdapter the DND
    // preference silently never reached disk.
