...
function glyphFromHints(hints) {
  return stringHint(hints, "dhms-glyph")
}

// Shell command to run when the card is clicked, sent by
// dhms-notification-send --exec. Carrying the action as data means it
...
function execFromHints(hints) {
  return stringHint(hints, "dhms-exec")
}
...
// ~/.local/state/dhms/notifications/ so toasts survive shell restarts
// (e.g. the restart `dhms-update` performs). The file exists exactly as
...
// senders (all dhms web apps) delete their scoped /tmp files on close,
