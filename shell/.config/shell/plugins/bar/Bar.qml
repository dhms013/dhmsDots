...
Item {
  ...
  // The dhms-shell host injects dotsPath from DHMSDOTS_PATH.
  required property string dotsPath
  ...
  property string qsConfigDir: home + "/.config/dhms"
  ...
  // to "monospace"), so changing the system font (via `dhms-font-set`)
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "dhms.bluetooth").
  ...
  // Contrast pick for transparent-bar text, computed inline (the omarchy
  // original shelled out to a helper script). Relative luminance decides
  // which candidate reads better against the desktop showing through.
  function relativeLuminance(colorValue) {
    function channel(value) {
      var v = Util.clamp(value, 0, 1)
      return v <= 0.03928 ? v / 12.92 : Math.pow((v + 0.055) / 1.055, 2.4)
    }
    return 0.2126 * channel(colorValue.r) + 0.7152 * channel(colorValue.g) + 0.0722 * channel(colorValue.b)
  }
  ...
  function refreshTransparentForeground() {
    ...
    var themeRatio = contrastRatio(themeForeground, Qt.rgba(0.5, 0.5, 0.5, 1))
    var candidate = themeForeground
    // The desktop behind the bar is unknown; compare both theme candidates
    // against mid-gray and keep whichever keeps more separation from it,
    // breaking ties toward the theme foreground.
    if (contrastRatio(themeContrastForeground, Qt.rgba(0.5, 0.5, 0.5, 1)) > themeRatio + 0.5)
      candidate = themeContrastForeground

    foregroundAnimationEnabled = false
    transparentForeground = candidate
  ...
  FileView {
    path: root.home + "/.config/themes/current"
    watchChanges: true
    printErrors: false
  ...
  // and the flag is created/removed by `dhms-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/dhms/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/dhms/toggles"
    watchChanges: true
    printErrors: false
    onFileChanged: {
      // Directory watches die on the first atomic shuffle inside (and
      // quickshell drops them after inode churn) — re-arm before probing.
  ...
  // Periodic safety net: if the directory watch dies between events the flag
  // state would otherwise stick until the next toggle.
  Timer {
    interval: 5000
    running: true
    repeat: true
    onTriggered: barHiddenProbe.running = true
  }
  ...
  component BarPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "dhms-bar"
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "dhms-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "dhms-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
