...
Item {
  ...
  function logEvent(event) {
    ...
    console.log("dhms lock " + lastEventAt + " " + event)
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "dhms-lock-preview"
  ...
  Process {
    ...
    command: ["bash", "-c", "if [[ -f /etc/pam.d/dhms-lock-fingerprint ]] && command -v fprintd-list >/dev/null 2>&1 && fprintd-list \"$USER\" 2>/dev/null | grep -qi finger; then echo yes; else echo no; fi"]
  ...
  Process {
    ...
    command: ["bash", "-c", "dhms-hyprland-session-locked"]
  ...
  Process {
    id: wakeProcess
    command: ["bash", "-c", "dhms-system-wake"]
  }

  Process {
    id: blankProcess
    command: ["bash", "-c", "dhms-brightness-keyboard off; dhms-brightness-display off"]
  }
  ...
  FileView {
    path: "/etc/pam.d/dhms-lock-password"
    watchChanges: true
    printErrors: false
