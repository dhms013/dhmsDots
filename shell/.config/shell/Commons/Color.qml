...
QtObject {
    ...
    readonly property string currentThemePath: home + "/.config/themes/current/theme"
    ...
    bar: QtObject {
        property color background: root.composed("bar.background", "bar.background-alpha", root.background, 1)
    ...
    readonly property QtObject
    popups: QtObject {
        property color background: root.composed("popups.background", "popups.background-alpha", root.background, 1)
        property color text: root.pick("popups.text", root.foreground)
        property color border: root.composed("popups.border", "popups.border-alpha", root.accent, 1)
    }

    readonly property QtObject
    tooltip: QtObject {
        property color background: root.composed("tooltip.background", "tooltip.background-alpha", root.background, 1)
        property color text: root.pick("tooltip.text", root.foreground)
        property color border: root.composed("tooltip.border", "tooltip.border-alpha", root.foreground, 1)
    }

    readonly property QtObject
    notifications: QtObject {
        property color background: root.composed("notifications.background", "notifications.background-alpha", root.background, 1)
        property color text: root.pick("notifications.text", root.foreground)
        property color border: root.composed("notifications.border", "notifications.border-alpha", root.accent, 1)
        property color countdown: root.pick("notifications.countdown", root.accent)
    }

    readonly property QtObject
    menu: QtObject {
        property color background: root.composed("menu.background", "menu.background-alpha", root.background, 1)
        property color text: root.pick("menu.text", root.foreground)
        property color border: root.composed("menu.border", "menu.border-alpha", root.foreground, 1)
        ...
        property color selectedBorder: root.composed("menu.selected-border", "menu.selected-border-alpha", root.foreground, 0)
    }
    ...
    readonly property QtObject
    polkit: QtObject {
        property color background: root.composed("polkit.background", "polkit.background-alpha", root.background, 1)
        ...
        property color border: root.composed("polkit.border", "polkit.border-alpha", root.accent, 1)
        property color borderError: root.composed("polkit.border-error", "polkit.border-alpha", root.urgent, 1)
    ...
    lock: QtObject {
        ...
        property color border: root.composed("lock.border", "lock.border-alpha", root.foreground, 1)
        property color borderActive: root.composed("lock.border-active", "lock.border-alpha", root.accent, 1)
        property color borderError: root.composed("lock.border-error", "lock.border-alpha", root.urgent, 1)
    ...
    readonly property QtObject
    imagePicker: QtObject {
        property color scrim: root.composed("image-picker.scrim", "image-picker.scrim-alpha", root.background, 0.87)
        property color text: root.pick("image-picker.text", root.foreground)
        property color selectedBorder: root.composed("image-picker.selected-border", "image-picker.selected-border-alpha", root.accent, 1)
    ...
    // is where `dhms display text size` writes `[font] base-size`. Watched so the
    // CLI takes effect live without restarting the shell; absent by default.
    property FileView userShellFile

    userShellFile: FileView {
        id: userShellFile

        path: root.home + "/.config/dhms/shell.toml"
        watchChanges: true
        printErrors: false
    ...
    function loadColors(raw) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (match[1] === "foreground") {
                ...
                accent = match[2];
                foundAccent = true;
            } else if (match[1] === "muted") {
                muted = match[2];
                foundMuted = true;
            } else if (match[1] === "color0")
                color0Value = match[2];
            else if (match[1] === "color4")
                color4Value = match[2];
            else if (match[1] === "color7")
                color7Value = match[2];
            else if (match[1] === "color8")
                color8Value = match[2];
        ...
        if (!loadedForeground && color7Value.length > 0)
            foreground = color7Value;

        if (!foundAccent && color4Value.length > 0)
            accent = color4Value;

        if (!foundMuted)
            muted = color8Value.length > 0 ? color8Value : foreground;
    ...
    // Style. User keys win, so a machine-level `~/.config/dhms/shell.toml`
