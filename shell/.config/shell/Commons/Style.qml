...
QtObject {
  ...
  // component follows the fontconfig alias `dhms-font-set` writes.
  ...
  // alias path keeps working when the user runs `dhms font set`), but
  ...
  // DHMS_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("DHMS_MENU_FONT")
  ...
  // Resolve the fontconfig alias to a concrete family name. `dhms font
  ...
  // `dhms toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/dhms/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
