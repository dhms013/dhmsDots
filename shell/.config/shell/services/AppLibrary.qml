...
Item {
  ...
  function remove(desktopId, name) {
    // Hide, don't uninstall: append to config/launcher.hides so the entry
    // vanishes from launcher views but stays installed and launchable.
    // omarchy's script goes further (pacman -Rns / flatpak uninstall); that
    // needs sudo prompts and is deliberately out of scope here.
    var id = root.normalizeDesktopId(desktopId)
    if (!id.length) return

    var lines = String(hidesFile.text() || "").split(/\n/)
    var next = []
    for (var i = 0; i < lines.length; i++) {
      var line = root.normalizeDesktopId(lines[i])
      if (line.length > 0 && line !== id) next.push(line)
    }
    next.push(id)
    var updated = next.join("\n") + "\n"
    hidesFile.setText(updated)
    // On quickshell 0.3.x setText only stages the text; writeAdapter puts it
    // on disk (there is no save()). Feed the parser the exact lines we just
    // wrote — text() still returns the previously loaded content here.
    hidesFile.writeAdapter()
    root.loadConfiguredHides(updated)
  }
  ...
  function hiddenEntryScanCommand() {
    ...
    var script = Quickshell.shellDir + "/services/hidden-entries.sh"
  ...
  function closeLaunchFeedback(serial) {
    ...
    if (root.launchOsdOpen) {
      Quickshell.execDetached(["dhms-shell", "osd", "close"])
  ...
  FileView {
    id: hidesFile
    // Same resolution as hiddenEntryScanCommand(): Quickshell.shellDir points
    // at the deployed config dir. (A bare shellPath here would fall through
    // the context chain and silently yield "undefined/config/...".)
    path: Quickshell.shellDir + "/config/launcher.hides"
    watchChanges: true
    atomicWrites: true
    printErrors: false
    onLoaded: root.loadConfiguredHides(text())
    // atomicWrites swaps the inode on every save, which silently kills the
    // inotify watch (same trap as the wallpaper symlink). Re-arm, then reload
    // explicitly — text() only reflects freshly read content after reload().
    onFileChanged: {
      hidesFile.watchChanges = false
      hidesFile.watchChanges = true
      hidesFile.reload()
    }
  ...
  Timer {
    ...
    onTriggered: {
      ...
      Quickshell.execDetached(["dhms-shell", "osd", "show", JSON.stringify({ icon: "󱓞", message: root.launchOsdMessage, duration: 0 })])
    }
  }
