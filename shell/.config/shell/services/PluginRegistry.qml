...
QtObject {
  ...
  property string pluginsDir: home + "/.config/dhms/plugins"
  ...
  //   - the built-in bar option (`dhms.bar`) is active when `bar.id` is
  //     missing or set to `dhms.bar`.
  ...
  //     silently make `dhms launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "dhms.bar"
  ...
  function resolveEnabledId(id) {
    ...
    for (var candidate in installedPlugins) {
      ...
      var metadata = manifest && Util.isPlainObject(manifest.dhms) ? manifest.dhms : null
  ...
  // back, and so a plugin that is both a widget and a menu (dhms.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "dhms.workspaces", center: "dhms.weather", right: "dhms.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.dhms)
        ? candidateManifest.dhms : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "dhms.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "dhms.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `dhms.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("dhms.") === 0) {
        console.warn("PluginRegistry: plugin " + tk
          + " rejected: id is reserved for first-party dhms plugins")
        continue
      }
  ...
  property Process localPluginWatcher: Process {
    // inotify-tools is optional: without it the shell would otherwise log a
    // start failure every second. Park on `sleep infinity` when the binary is
    // missing so third-party plugins are simply picked up on next restart.
    command: [
      "bash", "-c",
      "command -v inotifywait >/dev/null 2>&1 || sleep infinity; "
        + "exec inotifywait -m -r -q -e close_write,create,delete,move --format %w%f "
        + Util.shellQuote(registry.pluginsDir)
    ]
    stdout: SplitParser {
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/dhms/plugins.
    var script = ""
