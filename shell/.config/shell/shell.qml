...
ShellRoot {
  ...
  // The dhms-shell host is the long-running entry point. Plugins live in
  // sibling directories under plugins/. DHMSDOTS_PATH is provided by the
  // session environment when set; otherwise the checkout lives at
  // $HOME/.dhmsDots, which is where this repo is cloned.
  property string dotsPath: Quickshell.env("DHMSDOTS_PATH") || (home + "/.dhmsDots")
  readonly property string shellPath: Quickshell.shellDir
  readonly property string firstPartyPluginsDir: shellPath + "/plugins"
  readonly property string defaultsPath: shellPath + "/config/shell.json"
  readonly property string userConfigPath: home + "/.config/shell/shell.json"
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      ...
      layout: {
        ...
        center: [{ id: "dhms.clock", format: "dddd HH:mm" }],
        right: [{ id: "dhms.audio" }]
      }
    },
    plugins: []
  ...
  Component.onCompleted: {
    console.log("dhms-shell paths",
      "dotsPath=" + shell.dotsPath,
  ...
  readonly property string defaultBarId: "dhms.bar"
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("dotsPath" in target) target.dotsPath = shell.dotsPath
  ...
  function ensureService(pluginId) {
    ...
    function finalize() {
      ...
      if ("dotsPath" in inst) inst.dotsPath = shell.dotsPath
  ...
  function isBarWidgetPanelPlugin(pluginId) {
    ...
    // panel loader (e.g. dhms.menu); let that path handle them.
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("dotsPath" in item) item.dotsPath = shell.dotsPath
  ...
  function imagePickerItem() {
    var loader = panelLoaders["dhms.image-picker"]
  ...
  IpcHandler {
    ...
    // Programmatic apply used by scripts/tests: same flow as pressing Enter
    // on a selected thumbnail, minus UI selection state.
    function apply(path: string): string {
      var picker = shell.imagePickerItem()
      if (picker && typeof picker.applyExternal === "function") {
        picker.applyExternal(path)
        return "ok"
      }
      return "unknown"
    }
  ...
  function appLibraryItem() {
    return shell.appLibrary || null
  }
  ...
  IpcHandler {
    ...
    // Hide a desktop entry from launcher views (config/launcher.hides).
    // Same path as the menu's Remove action; useful for scripted curation.
    function hide(id: string): string {
      var library = shell.appLibraryItem()
    ...
    function hidden(id: string): string {
      ...
      return library.configuredHiddenEntryIds[library.normalizeDesktopId(id)] === true ? "yes" : "no"
    }
  }
